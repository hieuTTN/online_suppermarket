...
public interface UserRepository extends JpaRepository<User,Long> {
    ...
    @Query("select u from User u where u.authorities.name = ?2 and (u.email like ?1 or u.fullName like ?1 or u.phone like ?1 )")
    Page<User> getUserByRole(String role, Pageable pageable);

    @Query("select u from User u where u.email like ?1 or u.fullName like ?1 or u.phone like ?1 ")
